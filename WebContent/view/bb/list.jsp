...
<table border="">
		...
		<c:otherwise>
			<c:forEach items="${data }" var="ee" varStatus="no">
				<tr>
					...
					<td><c:if test="${ee.lev>0 }">
							<c:forEach begin="1" end="${ee.lev }">
		&nbsp;&nbsp;
		</c:forEach>
		└
		</c:if> <a href="Detail?id=${ee.id }&page=${page}">${ee.title }</a></td>
					<td>${ee.pname }</td>
					<td><fmt:formatDate value="${ee.reg_date }"
							pattern="yyyy-MM-dd" /></td>
				</tr>
			</c:forEach>

			<tr>
				<td colspan="4" align="center"><c:if test="${startPage >1}">
	...
	<tr>
		<td colspan="4" align="right"><a href="InsertForm?page=${page}">글쓰기</a>
		</td>
	</tr>
</table>